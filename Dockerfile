...
RUN wget https://dlcdn.apache.org/tomcat/tomcat-9/v9.0.85/bin/apache-tomcat-9.0.85.tar.gz && \
    ...
    mkdir -p /usr/local/ && \
    mv apache-tomcat-9.0.85 /usr/local/
...
RUN groupadd tomcat && useradd -s /bin/false -g tomcat -d /usr/local/apache-tomcat-9.0.85 tomcat
RUN chown -RH tomcat: /usr/local/apache-tomcat-9.0.85 && chmod +x /usr/local/apache-tomcat-9.0.85/bin/*.sh
...
CMD ./gradlew clean build && \
    ...
    /usr/local/apache-tomcat-9.0.85/bin/catalina.sh run
